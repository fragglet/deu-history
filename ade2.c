/*
   ...
   Doom II-specific code written by Adrian Cable.
   ...
   ADE2.C - Main program execution routines.

   Compile with command
      tcc -Z -mh -r -G -O -f -eade2 *.c \tc\lib\graphics.lib
...
int  MoveSpeed = 20;            /* number of pixels to scroll when pointer at edge of screen */
Bool AutoBSP = FALSE;           /* run BSP automatically after a node-build? */
...
char *BSPprog = "BSP";                  /* BSP program name */
...
/*   short & long names   type            message if true/changed       message if false              where to store the value */
   ...
   { NULL, "scrollpixels",OPT_INTEGER,    "Map scroll pixels",          NULL,                         &MoveSpeed      },
...
{
   fprintf( where, "Usage:\n");
   fprintf( where, "DEU2 [-w <main_wad_file>] [-d] [-sb] [-c] [-q] [-qq] [-e] [-a] [-i] [-z <zoom>] [-bgi <driver>] [-v <mode>] [-fc] [-config <ini_file>] [-pw <pwad_file>] [-file <pwad_files>...]\n");
...
{
// fprintf( where, "\nDEU II: Doom Editor Utilities for Doom II, ver %sáFinal[4].\n", DEU_VERSION);
// fprintf( where, " By Rapha‰l Quinet (quinet@montefiore.ulg.ac.be),\n");
// fprintf( where, "    Brendon J Wyber (b.wyber@csc.canterbury.ac.nz),\n");
// fprintf( where, "and Adrian Cable (acable@sound.demon.co.uk).\n\n");
   fprintf( where, "\nADE II: Amazing Doom Editor for Doom II, ver %s.\n", DEU_VERSION);
   fprintf( where, " By Rapha‰l Quinet (quinet@montefiore.ulg.ac.be),\n");
   fprintf( where, "    Brendon J Wyber (b.wyber@csc.canterbury.ac.nz),\n");
   fprintf( where, "and Adrian Cable (acable@sound.demon.co.uk).\n\n");
   fprintf( where, "Please address all comments/criticisms to Adrian Cable!\n\n");
}
...
void FunnyMessage( FILE *where)
{
   fprintf( where, "\n");
   fprintf( where, "*----------------------------------------------------------------------------*\n");
   fprintf( where, "| Welcome to ADE 2!  This is a powerful utility and, like all good tools, it |\n");
   fprintf( where, "| comes with a user's manual.  Please print and read ADE2.TXT if you want    |\n");
   fprintf( where, "| to discover all the features of this program.  If you are new to ADE, the  |\n");
   fprintf( where, "| tutorial will show you how to build your first level.                      |\n");
   fprintf( where, "|                                                                            |\n");
   fprintf( where, "| If you are an experienced ADE user and want to know what has changed since |\n");
...
void MainLoop()
{
   ...
   for (;;)
   {
      ...
      else if (!strcmp( com, "?"))
      {
         ...
         printf( "C[reate] [mapnumber]              -- to create and edit a new (empty) level\n");
         printf( "D[ump] <DirEntry> [outfile]       -- to dump a directory entry in hex\n");
         printf( "E[dit] [mapnumber]                -- to edit a game level saving results to\n");
         ...
         printf( "R[ead] <WadFile>                  -- to read/convert a Doom I or Doom II PWAD\n");
      ...
      else if (!strcmp( com, "WADS") || !strcmp( com, "W"))
      {
         ...
         {
            if ((wad->directory[ 0].name[ 0] == 'M' && wad->directory[ 0].name[ 1] == 'A' && wad->directory[ 0].name[ 2] == 'P') && (strncmp( wad->filename, "$TMP$", 5)))
               printf( "%-20s  PWAD  (Patch wad file for Doom II map %c%c)\n", wad->filename, wad->directory[ 0].name[ 3], wad->directory[ 0].name[ 4]);
            else
            {
               /* kluge */
               strncpy( input, wad->directory[ 0].name, 8);
               input[ 8] = '\0';
               if (strncmp( wad->filename, "$TMP$", 5))
      ...
      else if (!strcmp( com, "EDIT") || !strcmp( com, "E") || !strcmp( com, "CREATE") || !strcmp( com, "C"))
      {
         ...
         mapno = atoi(com);
         maptmp = (float)mapno;
         episode = ((int)maptmp/10);
         level = mapno%10;
         curlev = (episode*10)+level;
         com = strtok( input, " ");
      ...
      else if (!strcmp( com, "XTRACT") || !strcmp( com, "EXTRACT") || !strcmp( com, "X"))
      {
         ...
         if (wad)
         {
            printf( "[You cannot overwrite an opened Wad file with raw data.]\n");
            continue;
         }
         printf( "Saving directory entry data to \"%s\".\n", out);
         if ((file = fopen( out, "wb")) == NULL)
